...
>
> **Publishing** is audited separately in `publish-readiness.md` — the leaks, licence, onboarding, packaging and safety gaps that block a public repo or a downloadable build. Not scheduled work until publishing is a goal.
...
### Worktrees & Checkout
- **Worktree lifecycle status (thread 2).** A ticket-shaped, user-set status per worktree — in progress / on hold / PR'd / awaiting release / … — closer in function to a Linear ticket status than to anything runtime. Needs scoping (vocabulary, who sets it and when, whether any of it can be inferred from git/PR state, where it shows). **The persisted `Worktree.status: "ongoing" | "completed"` field is reserved for this**: it currently has no writer, and its only readers are the slot picker's `ongoing` filter and `initSlots`. Deliberately kept separate from the derived, session-only `activity` marker shipped 2026-08-20 (displayed / running / paused) — see the CLAUDE.md note for why the two must not share a name.

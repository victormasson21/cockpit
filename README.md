...
**London map · live** is the same map with real Underground trains on it, and it is the one background
...
before the first response, it renders as precisely the static **London map**; that is why it is a
separate entry in the picker rather than a switch on the static one, so choosing a background never
starts network activity you didn't ask for. Its train motion is baked by

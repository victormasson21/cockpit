...
41 Rust tests + 22 JS tests green; Rust + Vite builds clean. **`SLACK_ALLOWED_TOOLS`/`SLACK_MODEL` are committed
plan defaults — pending a human-run live smoke to pin and verify** (unlike Linear's consts, which are pinned).
GUI + live MCP acceptance PENDING human eyeball.

**Next:** sub-project 4 — auth manager + first read-only integration tile (provider + panel pattern; Keychain token
storage; the deferred `linear.rs` / `slack.rs` Rust provider swap points land here).
